#!/usr/bin/env python3
"""Build the v4 photo puppet (head / neck / body / wing / bill / legs / tail) from the master cut-out.
...
Layers (each cropped to its alpha bounds; offsets are recorded in manifest.json):
  ...
  neck            stretchable neck band between the shoulders and the skull; drawn over
                  the body and under the head, so the head can glide on a real neck
  wing            copy of the folded near wing, rotated at the shoulder for flapping
  head            skull only (soft collar).  The bill footprint is painted as dark
                  mouth interior near the gape and transparent beyond.
...
# Neck band: its own stretchable layer between the shoulders and the skull.
NECK = [
    (396, 300), (404, 360), (420, 430), (458, 492), (540, 520), (640, 505), (724, 452),
    (766, 380), (774, 300), (754, 228), (714, 168), (668, 120), (610, 96), (540, 100),
    (480, 130), (432, 180), (404, 240),
]
NECK_BASE = (658.0, 448.0)      # rides on the body (shoulders)
NECK_TOP = (588.0, 208.0)       # the head hangs off this point
WING = [
    (640, 268), (700, 208), (790, 178), (920, 172), (1050, 196), (1160, 250), (1250, 330),
    (1320, 430), (1360, 520), (1355, 560), (1300, 566), (1180, 552), (1020, 520),
    (860, 470), (740, 405), (664, 330),
]
WING_PIVOT = (700.0, 240.0)
...
def main():
    ...
    head_region = soften(skull, HEAD_FEATHER)
    ...
    # --- neck and wing (both are soft copies drawn between body and head) ----
    neck_layer = rgba_layer(source, alpha * soften(poly_mask(NECK), 26.0))
    wing_layer = rgba_layer(source, alpha * soften(poly_mask(WING), 18.0))
